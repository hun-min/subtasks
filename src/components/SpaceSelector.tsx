...
import { Plus, ChevronDown, Check, Layout } from 'lucide-react';

export function SpaceSelector() {
  const { spaces, currentSpace, setCurrentSpace, addSpace } = useSpace();
      ...
      {isOpen && (
        <div className="absolute top-full left-0 mt-2 w-56 bg-[#18181b] border border-white/10 rounded-xl shadow-2xl p-1 z-50 animate-in fade-in zoom-in-95 duration-100 origin-top-left">
          <div className="max-h-60 overflow-y-auto scrollbar-hide py-1">
            {spaces.map(space => (
              <button key={space.id} onClick={() => { setCurrentSpace(space); setIsOpen(false); }} className={`w-full text-left px-3 py-2 rounded-lg text-sm flex items-center justify-between group transition-colors ${currentSpace?.id === space.id ? 'bg-blue-600/10 text-blue-400' : 'text-gray-400 hover:bg-white/5 hover:text-gray-200'}`}>
