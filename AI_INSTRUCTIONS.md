# AI 개발 및 배포 가이드 (절대 지침)

이 파일은 이 프로젝트를 작업하는 모든 AI(Roo, Cursor 등)가 반드시 준수해야 하는 **최우선 지침서**이다. 사용자의 의도를 오해하거나 반복된 질문으로 시간을 끄는 것을 방지하기 위해 작성되었다.
...
## 2. UI/UX 디자인 절대 규칙
### 캘린더 (Calendar)
- **스트릭 표시**: 날짜 칸 내부 **우측 상단(`absolute top-0.5 right-0.5`)**에 불꽃 아이콘(`Flame`, size 10)과 **흰색 숫자**(`text-white`, size 9)를 반드시 함께 표시한다. 아이콘과 숫자가 서로 가려지지 않게 `gap-0.5`를 유지한다.
