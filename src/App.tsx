...
function UnifiedTaskItem({ 
  ...
  const paddingLeft = currentDepth * 20;
  ...
  return (
    <div ref={setNodeRef} style={style} className={`relative group flex items-start gap-2 py-0 px-4 transition-colors ${isFocused ? 'bg-white/[0.04]' : ''}`} onTouchStart={handleItemTouchStart} onTouchEnd={handleItemTouchEnd}>
      ...
      <div className="flex flex-col items-center justify-start mt-[8px]">
        <button onClick={() => updateTask({ ...task, status: task.status === 'DONE' ? 'LATER' : 'DONE', isTimerOn: false })} className={`flex-shrink-0 w-[16px] h-[16px] border-[1.2px] rounded-[4px] flex items-center justify-center transition-all ${getStatusColor()}`}>
      ...
      <div className="flex-1 relative">
        ...
        {suggestions.length > 0 && (
          <div className="absolute left-0 top-full z-[110] mt-0.5 bg-[#1a1a1f] border border-white/10 rounded-lg shadow-2xl overflow-hidden min-w-[200px]">
            {suggestions.map((s, idx) => <button key={idx} onClick={() => { updateTask({ ...task, text: s.text }); setSuggestions([]); }} className={`w-full px-3 py-1 text-left text-sm ${selectedSuggestionIndex === idx ? 'bg-blue-600 text-white' : 'text-gray-400 hover:bg-white/5'}`}>{s.text}</button>)}
          </div>
        )}
      </div>
      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity mt-[6px]">
        <button onClick={() => deleteTask(task.id)} className="text-gray-600 hover:text-red-500 p-0.5"><X size={14} /></button>
        <div {...attributes} {...listeners} className="w-5 h-5 cursor-grab text-gray-700 flex items-center justify-center outline-none touch-none"><List size={14} /></div>
      </div>
    </div>
  );
}
