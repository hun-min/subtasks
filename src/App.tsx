...
import { Play, Pause, BarChart2, X, Check, ChevronLeft, ChevronRight, Plus, Calendar, List, Clock, Eye, EyeOff, ArrowRight, ArrowLeft, RotateCcw, RotateCw } from 'lucide-react';
...
function UnifiedTaskItem({ 
  ...
  const paddingLeft = currentDepth * 28;
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    if (e.key === 'Backspace' && task.text === '') { e.preventDefault(); if (currentDepth > 0) onOutdent?.(); else deleteTask(task.id); return; }
  ...
  return (
    <div ref={setNodeRef} style={style} className={`relative group flex items-start gap-3 py-0 px-4 transition-colors ${isFocused ? 'bg-white/[0.04]' : ''}`} onTouchStart={handleItemTouchStart} onTouchEnd={handleItemTouchEnd}>
      ...
      <div className="flex flex-col items-center justify-start mt-[11px]">
        <button onClick={() => updateTask({ ...task, status: task.status === 'DONE' ? 'LATER' : 'DONE', isTimerOn: false })} className={`flex-shrink-0 w-[19px] h-[19px] border-[1.5px] rounded-[5px] flex items-center justify-center transition-all ${getStatusColor()}`}>
          {task.status === 'DONE' && <Check size={13} className="text-white stroke-[3]" />}
          {task.isTimerOn && <div className="w-1.5 h-1.5 bg-white rounded-full animate-pulse" />}
        </button>
      </div>
      <div className="flex-1 relative">
        <AutoResizeTextarea value={task.text} autoFocus={isFocused} onFocus={() => setFocusedTaskId(task.id)} onChange={(e: any) => updateTask({ ...task, text: e.target.value })} onKeyDown={handleKeyDown} className={`w-full text-[17px] font-medium leading-[1.6] py-2 ${task.status === 'DONE' ? 'text-gray-500 line-through decoration-[1.5px]' : 'text-[#e0e0e0]'}`} placeholder="" />
        {suggestions.length > 0 && (
          <div className="absolute left-0 top-full z-[110] mt-1 bg-[#1a1a1f] border border-white/10 rounded-lg shadow-2xl overflow-hidden min-w-[200px]">
            {suggestions.map((s, idx) => <button key={idx} onClick={() => { updateTask({ ...task, text: s.text }); setSuggestions([]); }} className={`w-full px-3 py-2 text-left text-sm ${selectedSuggestionIndex === idx ? 'bg-blue-600 text-white' : 'text-gray-400 hover:bg-white/5'}`}>{s.text}</button>)}
          </div>
        )}
      </div>
      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity mt-[10px]">
        <button onClick={() => deleteTask(task.id)} className="text-gray-600 hover:text-red-500 p-1"><X size={16} /></button>
        <div {...attributes} {...listeners} className="w-6 h-6 cursor-grab text-gray-700 flex items-center justify-center outline-none touch-none"><List size={16} /></div>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-[#e0e0e0] selection:bg-[#7c4dff]/30 font-sans overflow-x-hidden">
      ...
      <div className="max-w-xl mx-auto min-h-screen flex flex-col p-4 pb-32">
        <div className="mb-4 flex justify-between items-center"><SpaceSelector /><div className="flex gap-4 items-center"><button onClick={handleUndo} disabled={historyIndex <= 0} className="text-gray-600 disabled:opacity-20"><RotateCcw size={18} /></button><button onClick={handleRedo} disabled={historyIndex >= history.length - 1} className="text-gray-600 disabled:opacity-20"><RotateCw size={18} /></button><button onClick={() => user ? signOut() : setShowAuthModal(true)} className="text-xs text-gray-500 hover:text-white">{user ? 'Logout' : 'Login'}</button></div></div>
        
        <div className="calendar-area mb-8 bg-[#0f0f14] p-5 rounded-3xl border border-white/5 shadow-2xl" onTouchStart={(e) => swipeTouchStart.current = e.touches[0].clientX} onTouchEnd={(e) => { if (swipeTouchStart.current === null) return; const diff = swipeTouchStart.current - e.changedTouches[0].clientX; if (Math.abs(diff) > 100) setViewDate(new Date(viewDate.getFullYear(), viewDate.getMonth() + (diff > 0 ? 1 : -1), 1)); swipeTouchStart.current = null; }}>
        ...
        <div className="mb-10 text-center px-4"><div className="text-[52px] font-black mb-1 flex items-baseline justify-center text-white tracking-tighter">{doneCount}<span className="text-[28px] text-gray-600 font-light mx-2">/</span>{tasks.length}<span className="text-[20px] text-[#7c4dff] font-black ml-2 tracking-normal">{completionRate}%</span></div><AutoResizeTextarea value={currentLog?.memo || ''} onChange={(e: any) => { const newMemo = e.target.value; setLogs(prev => { const updated = prev.map(l => l.date === viewDate.toDateString() ? { ...l, memo: newMemo } : l); saveToLocalStorage(updated); return updated; }); }} placeholder="+" className="w-full bg-transparent text-[18px] text-[#7c4dff] font-bold text-center outline-none placeholder:text-gray-800" /></div>

        <div className="flex-1 space-y-12">
          <div className="animate-in fade-in duration-500"><div className="flex items-center justify-between mb-4 px-3"><h2 className="text-[11px] font-black tracking-[0.25em] text-pink-500 uppercase flex items-center gap-2"><Clock size={16} className="animate-pulse" /> A SECOND</h2><button onClick={() => setIsSecondVisible(!isSecondVisible)} className="text-gray-600 hover:text-gray-400 transition-colors p-1">{isSecondVisible ? <Eye size={18} /> : <EyeOff size={18} />}</button></div>{isSecondVisible && <div className="space-y-0"><DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}><SortableContext items={secondTasks.map(t => t.id)} strategy={verticalListSortingStrategy}>{secondTasks.map((task, idx) => <UnifiedTaskItem key={task.id} task={task} index={idx} allTasks={secondTasks} updateTask={(u) => updateStateAndLogs(tasks.map(t => t.id === u.id ? u : t))} deleteTask={(id) => updateStateAndLogs(tasks.filter(t => t.id !== id))} focusedTaskId={focusedTaskId} setFocusedTaskId={setFocusedTaskId} logs={logs} onAddTask={handleAddTask} />)}</SortableContext></DndContext><button onClick={() => handleAddTask(undefined, true)} className="w-full py-2.5 border border-dashed border-pink-500/10 rounded-2xl text-gray-700 hover:text-pink-500/40 transition-all flex items-center justify-center gap-2 text-[11px] font-black mt-2 uppercase tracking-widest"><Plus size={14} /> Add Ritual</button></div>}</div>
          <div className="animate-in fade-in duration-700"><h2 className="text-[11px] font-black tracking-[0.25em] text-[#7c4dff] uppercase mb-4 px-3 flex items-center gap-2"><List size={16} /> FLOW</h2><div className="space-y-0"><DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}><SortableContext items={planTasks.map(t => t.id)} strategy={verticalListSortingStrategy}>{planTasks.map((task, idx) => <UnifiedTaskItem key={task.id} task={task} index={idx} allTasks={planTasks} updateTask={(u) => updateStateAndLogs(tasks.map(t => t.id === u.id ? u : t))} deleteTask={(id) => { const newTasks = tasks.filter(t => t.id !== id); updateStateAndLogs(newTasks); const idxInTasks = tasks.findIndex(t => t.id === id); if (idxInTasks > 0) setFocusedTaskId(tasks[idxInTasks-1].id); else if (newTasks.length > 0) setFocusedTaskId(newTasks[0].id); else setFocusedTaskId(null); }} focusedTaskId={focusedTaskId} setFocusedTaskId={setFocusedTaskId} logs={logs} onAddTask={handleAddTask} onIndent={() => { const actualIdx = tasks.findIndex(t => t.id === task.id); if (actualIdx > 0) updateStateAndLogs(tasks.map(t => t.id === task.id ? { ...t, depth: Math.min((tasks[actualIdx-1].depth || 0) + 1, (t.depth || 0) + 1) } : t)); }} onOutdent={() => updateStateAndLogs(tasks.map(t => t.id === task.id ? { ...t, depth: Math.max(0, (t.depth || 0) - 1) } : t))} onMoveUp={() => { const idx = tasks.findIndex(t => t.id === task.id); if (idx > 0) updateStateAndLogs(arrayMove(tasks, idx, idx - 1)); }} onMoveDown={() => { const idx = tasks.findIndex(t => t.id === task.id); if (idx < tasks.length - 1) updateStateAndLogs(arrayMove(tasks, idx, idx + 1)); }} />)}</SortableContext></DndContext><button onClick={() => handleAddTask(undefined, false)} className="w-full py-3.5 border border-dashed border-white/5 rounded-2xl text-gray-700 hover:text-gray-400 transition-all flex items-center justify-center gap-2 text-sm font-black mt-3"><Plus size={16} /> NEW FLOW</button></div></div>
        </div>

        {activeTask && <div className="fixed bottom-8 left-1/2 -translate-x-1/2 w-[94%] max-w-sm z-[200] animate-in slide-in-from-bottom-8 duration-500 cubic-bezier(0.16, 1, 0.3, 1)"><div className="bg-[#121216]/95 backdrop-blur-3xl border border-white/10 rounded-[28px] p-3 shadow-[0_25px_60px_rgba(0,0,0,0.6)] flex items-center justify-between gap-3"><div className="flex items-center gap-2"><button onClick={() => updateStateAndLogs(tasks.map(t => t.id === activeTask.id ? { ...t, isTimerOn: !t.isTimerOn, timerStartTime: !t.isTimerOn ? Date.now() : undefined } : t))} className={`p-3.5 rounded-2xl transition-all ${activeTask.isTimerOn ? 'bg-[#7c4dff] text-white shadow-[0_0_25px_rgba(124,77,255,0.5)] scale-110' : 'bg-white/5 text-gray-400 hover:text-white'}`}>{activeTask.isTimerOn ? <Pause size={22} fill="currentColor" /> : <Play size={22} fill="currentColor" />}</button><div className="h-10 w-px bg-white/10 mx-1.5" /><div className="flex flex-col"><span className="text-[9px] text-gray-500 font-black uppercase tracking-widest mb-0.5">Time</span><input type="text" value={formatTime(activeTask.actTime)} onChange={(e) => updateStateAndLogs(tasks.map(t => t.id === activeTask.id ? { ...t, actTime: parseTimeToSeconds(e.target.value) } : t))} className="bg-transparent text-[18px] font-black font-mono text-[#7c4dff] outline-none w-24 tracking-tight" /></div></div><div className="flex gap-1"><button onMouseDown={(e) => e.preventDefault()} onClick={() => { const actualIdx = tasks.findIndex(t => t.id === activeTask.id); if (actualIdx > 0) updateStateAndLogs(tasks.map(t => t.id === activeTask.id ? { ...t, depth: Math.min((tasks[actualIdx-1].depth || 0) + 1, (t.depth || 0) + 1) } : t)); }} className="p-2.5 rounded-xl bg-white/5 text-gray-400 active:bg-white/10"><ArrowRight size={20} /></button><button onMouseDown={(e) => e.preventDefault()} onClick={() => updateStateAndLogs(tasks.map(t => t.id === activeTask.id ? { ...t, depth: Math.max(0, (t.depth || 0) - 1) } : t))} className="p-2.5 rounded-xl bg-white/5 text-gray-400 active:bg-white/10"><ArrowLeft size={20} /></button></div><div className="flex items-center gap-1"><button onClick={() => setShowDatePicker(true)} className="p-2.5 rounded-xl hover:bg-white/5 text-gray-400 hover:text-white transition-colors"><Calendar size={20} /></button><button onClick={() => setShowHistoryTarget(activeTask.text)} className="p-2.5 rounded-xl hover:bg-white/5 text-gray-400 hover:text-white transition-colors"><BarChart2 size={20} /></button><button onClick={() => setFocusedTaskId(null)} className="p-2.5 rounded-xl hover:bg-white/5 text-gray-400 hover:text-white transition-colors"><X size={20} /></button></div></div></div>}

        {showDatePicker && activeTask && <div className="fixed inset-0 z-[300] bg-black/70 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in duration-200" onClick={() => setShowDatePicker(false)}><div className="bg-[#0a0a0f]/90 backdrop-blur-xl border border-white/10 rounded-3xl p-6 w-full max-w-sm shadow-2xl" onClick={(e) => e.stopPropagation()}><div className="flex justify-between items-center mb-4"><button onClick={() => setViewDate(new Date(viewDate.getFullYear(), viewDate.getMonth() - 1, 1))}><ChevronLeft size={20} className="text-gray-500" /></button><span className="font-bold text-white">{viewDate.toLocaleString('default', { month: 'long', year: 'numeric' })}</span><button onClick={() => setViewDate(new Date(viewDate.getFullYear(), viewDate.getMonth() + 1, 1))}><ChevronRight size={20} className="text-gray-500" /></button></div><div className="grid grid-cols-7 gap-2">{['S','M','T','W','T','F','S'].map((d, idx) => <div key={`day-${idx}`} className="text-center text-[10px] text-gray-600">{d}</div>)}{Array.from({ length: 35 }).map((_, i) => { const d = new Date(viewDate.getFullYear(), viewDate.getMonth(), 1); d.setDate(d.getDate() + (i - d.getDay())); return <button key={i} onClick={() => { const targetDate = d.toDateString(); const currentDate = viewDate.toDateString(); if (targetDate !== currentDate) { const newLogs = logs.map(l => { if (l.date === currentDate) return { ...l, tasks: l.tasks.filter(t => t.id !== activeTask.id) }; if (l.date === targetDate) return { ...l, tasks: [...l.tasks, { ...activeTask, id: Date.now() }] }; return l; }); if (!newLogs.some(l => l.date === targetDate)) newLogs.push({ date: targetDate, tasks: [{ ...activeTask, id: Date.now() }], memo: '' }); setLogs(newLogs); saveToLocalStorage(newLogs); setTasks(prev => prev.filter(t => t.id !== activeTask.id)); setFocusedTaskId(null); } setShowDatePicker(false); }} className={`aspect-square rounded-lg border flex items-center justify-center hover:bg-blue-600/20 transition-colors ${d.toDateString() === new Date().toDateString() ? 'border-blue-500 text-blue-400' : 'border-gray-800 text-gray-400'}`}><span className="text-sm">{d.getDate()}</span></button>; })}</div></div></div>}
